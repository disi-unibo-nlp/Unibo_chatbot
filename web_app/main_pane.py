...
def setup_chat_environment():
    with st.spinner(text="Loading and indexing the data - hang tight! This should take 1-2 minutes."):
        ...
        #encoder_model_name = "BAAI/bge-small-en"
...
- se non conosci la risposta, rispondi che non sei in grado di fornire informazioni adeguate e consiglia di consultare il sito di ateneo;
...
if prompt := st.chat_input("La tua domanda ..."): 
    st.session_state.messages.append({"role": "user", "content": chat_role2tag['user'] + prompt})
...
if st.session_state.messages[-1]["role"] != "assistant":
    with st.chat_message("assistant", avatar="/CA_Synthetic/web_app/images/logo-unibo.png"):
        with st.spinner("Ci sto pensando su..."):
            ...
            reps = ['Reply:', "La risposta che ho ricevuto dall'AI è:", "Response"] 
            for rep in reps:
            ...
            message = {"role": "assistant", "content":  chat_role2tag['assistant'] + out_response}
